...
namespace ExportToExcel
{
    ...
    public class CreateExcelFile
    {
        ...
        public static bool CreateExcelDocument(DataTable dt, string filename, HttpResponseBase Response)
        {
            try
            {
                DataSet ds = new DataSet();
        ...
        public static byte[] CreateExcelDocument<T>(List<T> list, string filename, HttpResponseBase Response)
        {
          //  try
          //  {
                DataSet ds = new DataSet();
                ds.Tables.Add(ListToDataTable(list));
               return CreateExcelDocumentAsStream(ds, filename, Response);
               // return true;
            //}
          //  catch (Exception ex)
            //{
        ...
        public static byte[] CreateExcelDocumentAsStream(DataSet ds, string filename, HttpResponseBase Response)
        {
         //   try
           // {
                System.IO.MemoryStream stream = new System.IO.MemoryStream();
                //string scriptPath = Path.Combine(System.Web.HttpContext.Current.Server.MapPath("~/Cache/temp.xls"));
             ...
             using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                {
                    worksheet.Cells[1, i + 1].Value = ds.Tables[0].Columns[i];
                ...
                for (int i = 0; i < colCount; i++)
                {
                    worksheet.Cells[2, i + 1].Value = i.ToString();
                    worksheet.Cells[2, i + 1].Style.Fill.PatternType = ExcelFillStyle.Solid;
                    worksheet.Cells[2, i + 1].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.Green);
                ...
                for (int row =0; row < ds.Tables[0].Rows.Count; row++)
                {
                    for (int i = 0; i < colCount; i++)
                    {
                        worksheet.Cells[rowNumber, i + 1].Value = ds.Tables[0].Rows[row].ItemArray[i].ToString();
                    }
                    rowNumber++;
                }
      ...
      //      catch (Exception ex)
        ...
        private static void WriteDataTableToExcelWorksheet(DataTable dt, WorksheetPart worksheetPart)
        {
            ...
            for (int colInx = 0; colInx < numberOfColumns; colInx++)
            {
                ...
                IsNumericColumn[colInx] = (col.DataType.FullName == "System.Double") || (col.DataType.FullName == "System.Int32");
            }
            rowIndex++;
            var numerRow = new Row { RowIndex = rowIndex };  // add a row at the top of spreadsheet
            sheetData.Append(numerRow);
            for (int colInx = 0; colInx < numberOfColumns; colInx++)
            {
                DataColumn col = dt.Columns[colInx];
                AppendTextCell(excelColumnNames[colInx] + "2", colInx.ToString(), numerRow);
        ...
        private static void AppendTextCell(string cellReference, string cellStringValue, Row excelRow)
        {
          ...
          //  cellValue.InnerText = cellStringValue;
            cellValue.Text = XmlEscape(cellStringValue);

...
$(document).ready(function () {
    ...
    if (screen.height < anotherScreenResolutionY) {
        ...
        if ($(".header_main > div.nav").length > 0) {
            $(".midd_section").addClass("paddingtop");//css("padding-top", "61px");
        }
        else {
    ...
    function constructRouteHash(requestCase, productName) {
        ...
        if ((requestCase == knownRequestCases.CATEGORY || requestCase == knownRequestCases.SUBCATEGORY) && SearchProductData.FeaturedCategories && SearchProductData.FeaturedCategories != "") {
                newHref =
                "#!/" + SearchProductData.FeaturedCategories
                + "/" + SearchProductData.shopID
                + "?_=" + new Date().valueOf();

        } else {
                newHref =
                "#!/category/" + SearchProductData.shopID
                + "/" + (SearchProductData.categoryID?SearchProductData.categoryID:0)
                + "?_=" + new Date().valueOf();
        }
    ...
    $("li.nav1 > a").on("click", function () {
        ...
        //if (!$this.hasClass("first")) {
            ...
            if ($this.hasClass("first")) {
                SearchProductData.FeaturedCategories = $this.attr("id");
            } else {
                SearchProductData.FeaturedCategories = '';
            }
    ...
    $(".subCategory").on("click", function (e) {
        var $this = $(this);
            ...
            if ($this.hasClass("first")) {
                SearchProductData.FeaturedCategories = $this.find("a").attr("id");
            } else {
                SearchProductData.FeaturedCategories = '';
            }

   ...
   GetQuantityString:function (productModel){
                ...
                if (d.Delete && temp.length > 0) {
                    if (Shop.isMobile) {
                        ...
                        if (SearchProductData.viewIn == "table" || SearchProductData.viewIn == "tableItemPartial") {
                            ...
                            temp.closest(".product_box_open").find(".product_green_btn").hide();
                ...
                if (wrap) {
                    ...
                    if (data.data.Quantity == 0)
                    {
                        ...
                        if (Shop.isMobile) {
                            ...
                            $('.prdct_qty_part', quantityInput.parents(".sh_protidctItemWrap")).hide();
                            $('.productBuyBtn', quantityInput.parents(".sh_protidctItemWrap")).show();
                        }
                        else {
                            console.log(wrap);
                            ...
                            $('.prdct_qty_part', quantityInput.parents('.sh_protidctItemWrap:eq(0)')).hide();
                            $('.product_green_btn', quantityInput.parents('.sh_protidctItemWrap:eq(0)')).show();
                            ...
                            $('.cart2_left_qty_in', cartquantityInput).hide();
                            $('.product_green_btn3', cartquantityInput).show();
    ...
    BoxChangeQuantityType: function (e, event, id, type,price,priceunit,dimension,step) {
        ...
        if (type == 1) {
            ...
            $('#priceSelector_' + id, wrap).html('₪' + (nextquant * priceunit).toFixed(2) +( true ? ' / ' +  'יחידה' :''));
            if (item.length > 0) {
            ...
            var item = $(".stripmain").find("[data-productshopid='" + id + "']");
            $('#priceSelector_' + id, wrap).html('₪' + (val * price / step).toFixed(2) + (true ? ' / ' +  '' + dimension : ''));
            if (item.length > 0) {
                var _productID = item.data("productid");
                Shop.ShowLoading($(e).parents('.sh_protidctItemWrap:eq(0)'));
    ...
    BoxDecrease: function (e, event, id, changeInCart) {
        ...
        if (arguments.length > 6 && arguments[6]) {
            if (n == 0) { n = 1; }
            $('#priceSelector_' + id, wrap).html('₪' + (n * arguments[6]).toFixed(2) + (true ? ' / ' + 'יחידה' : ''));
        }
    },
            ...
            if (Shop.isMobile) {
                ...
                if (SearchProductData.viewIn == "table" || SearchProductData.viewIn == "tableItemPartial") {
                    ...
                    if (sel.length > 0) {
                        ...
                        //}
                        $('.prdct_qty_part', $(e).parents('.sh_protidctItemWrap:eq(0)')).show();
                        $('.product_green_btn', $(e).parents('.sh_protidctItemWrap:eq(0)')).hide();
    ...
    DoReg: function (e, event) {
        ...
        $.ajax({
            ...
            data: $(e).parents('form:eq(0)').serialize(),
            cache: false,
            error: function () { alert('Server error(registration)'); }
        }).done(function (data) {
            if (data.result == "ok") {
                while (p = Shop.queueEvent.pop()) {
                    p.func(p.data);
